...
class Celeba64Benchmark:
    ...
    def __init__(self, width, deg ,dim = 3*64*64,  semi_length_square = 1.2 , seed = 295, file_name = "../benchmarks/celeba/Final_celeba_G.pt" , device="cuda"):
        ...
        self.dim =  dim
        self.device = device
        self.resnet = load_resnet_G(file_name, shape = 3*64*64)
...
class Cifar32Benchmark:
    
    def __init__(self, width, deg ,dim = 3*32*32,  semi_length_square = 1.2 , seed = 295 , 
                 file_name = "../benchmarks/cifar/Final_cifar_G.pth", device="cuda"):
        
        folder = "../benchmarks/cifar/width_{}".format(width)
        filename = "deg_{}_seed_{}.pt".format(deg ,seed)
        net = MinFunnels(dim, width)
        path = os.path.join(folder,filename)
        if os.path.exists(path):
        ...
        self.dim =  dim
        self.device = device
        self.resnet = load_resnet_G(file_name, shape = 3*32*32)
